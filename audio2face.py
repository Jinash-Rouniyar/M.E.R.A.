...
class Audio2FaceService:
    ...
    def tts_to_wav(self, tts_byte, framerate=x) -> str:
        """
        :param tts_byte: tts data in byte
        :param framerate: framerate
        :return: wav byte
        """
        seg = AudioSegment.from_raw(io.BytesIO(tts_byte), sample_width=2, frame_rate=framerate, channels=1)
        wavIO = io.BytesIO()
    ...
    def make_avatar_speaks(self, audio,sample_rate) -> None:
        """
        ...
        x = sample_rate
        print(x)
        push_audio_track(self.a2f_url, self.get_tts_numpy_audio(audio), sample_rate, self.avatar_instance)
